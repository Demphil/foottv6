...
// --- 🌟 الدالة الجديدة لحل مشكلة توقيت ما بعد منتصف الليل 🌟 ---
function getCorrectMatchDate(timeStr) {
    ...
    // إذا كان التوقيت في الماضي بأكثر من 12 ساعة، فالمباراة فعلياً فجر الغد
    if (diffHours < -12) {
        matchDate.setDate(matchDate.getDate() + 1);
    } 
    // إذا كان التوقيت في المستقبل بأكثر من 12 ساعة، فالمباراة كانت البارحة ليلاً
...
function sortMatchesByPriority(a, b) {
    ...
    // استخدام الدالة الجديدة للحصول على التاريخ الصحيح
    const dateA = getCorrectMatchDate(a.time);
    const dateB = getCorrectMatchDate(b.time);
    
    const diffA = (dateA - now) / 60000; 
    const diffB = (dateB - now) / 60000;

    const getRank = (diff) => {
        if (diff < 0 && diff > -140) return 1; // جارية الآن
        if (diff >= 0 && diff <= 45) return 2; // ستبدأ قريباً
        if (diff > 45) return 3;               // قادمة
        return 4;                              // انتهت
    };
...
function renderMatch(match) {
  ...
  // استخدام الدالة الجديدة لحساب الوقت المتبقي بشكل دقيق
